...
class Dataset(object):
    ...
    def __init__(self, dataset_path, **kw):
        ...
        # lxc
        self.obj_bbs_x = np.empty( (self.noof_training_imgs, 4), dtype=np.int )
        self.obj_bbs_y = np.empty( (self.noof_training_imgs, 4), dtype=np.int )
        self.Rs = np.empty( (self.noof_training_imgs, 3, 3), dtype=np.float32 )
        self.rand_idcs = None
    ...
    def get_training_images(self, dataset_path, args, experiment_name=None, augment_data=False):
        '''
        two input params added by lxc: experiment_name=None, augment_data=False      
        '''
        # current_config_hash = hashlib.md5(str(args.items('Dataset')+args.items('Paths'))).hexdigest()
        current_config_hash = experiment_name+'_training_image' if experiment_name is not None else 'training_image'
        current_config_hash = current_config_hash + '_aug' if augment_data else current_config_hash + 'not_aug'
        ...
        if os.path.exists(current_file_name):
            ...
            self.render_training_images(dataset_path, experiment_name, augment_data=augment_data)
            np.savez(current_file_name, train_x = self.train_x, mask_x = self.mask_x, train_y = self.train_y, 
                    # lxc
                    obj_bbs_x=self.obj_bbs_x, obj_bbs_y = self.obj_bbs_y, Rs = self.Rs)
    ...
    def load_bg_images(self, dataset_path, experiment_name=None):
        # current_config_hash = hashlib.md5(str(self.shape) + str(self.noof_bg_imgs) + str(self._kw['background_images_glob'])).hexdigest()
        current_config_hash = experiment_name+'_bg_image' if experiment_name is not None else 'bg_image'
    ...
    def render_training_images(self, dataset_path = None, experiment_name=None, augment_data = False):
        '''
    ...
    def batch(self, batch_size):
        '''
        augment training image with background image
        '''
        ...
        rand_idcs_bg = np.random.choice(self.noof_bg_imgs, batch_size, replace=True) # replace = True, allow dulplicate background
        ...
        # needs uint8
        batch_x = self._aug.augment_images(batch_x) # needs uint8 input
        # print("batch_x.dtype", batch_x.dtype)
        #slow...
        batch_x = batch_x / 255.
        batch_y = batch_y / 255.
